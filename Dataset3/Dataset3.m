lncrnaDisease = load('LDA-LNSUBRW-known_lncRNA_disease_interaction.txt');  %115RNA 178dis

LNSUBRWmatrix = lncrnaDisease;
save LNSUBRWmatrix LNSUBRWmatrix;
...
[~,LNSUBRWlncrna] = xlsread('LDA-LNSUBRW-lncRNA_115.xlsx','Sheet1');
[~,LNSUBRWdisease] = xlsread('LDA-LNSUBRW-disease_178.xlsx','Sheet1');

association = cell(540,2);
for i=1:540
    i
    lncname=LNSUBRWlncrna{gld(i,1),1};
    disname=LNSUBRWdisease{gld(i,2),1};
    association{i,1}={lncname};
    association{i,2}={disname};
end
xlswrite('associationLNSUBRW.xlsx',association);
...
for cv=1:10
    ...
    for ccv=1:5
        ...
        if ccv==1
            ...
            xlswrite(['edge_f_lnsubrw_cv',num2str(cv),'.xlsx'],edge_f1,'Sheet 1')
        ...
        elseif ccv<5
            ...
            if ccv==2
                ...
                xlswrite(['edge_f_lnsubrw_cv',num2str(cv),'.xlsx'],edge_f2,'Sheet 2')
            elseif ccv==3
                ...
                xlswrite(['edge_f_lnsubrw_cv',num2str(cv),'.xlsx'],edge_f3,'Sheet 3')
            else ccv==4
                ...
                xlswrite(['edge_f_lnsubrw_cv',num2str(cv),'.xlsx'],edge_f4,'Sheet 4')
            end
            ...
            xlswrite(['edge_f_lnsubrw_cv',num2str(cv),'.xlsx'],edge_f5,'Sheet 5')
        end
...
LNSUBRW_dis_gsSim = disease_gsSim;
LNSUBRW_lnc_gsSim = lncRNA_gsSim;
LNSUBRWCoslnc = Coslnc;
LNSUBRWCosDis = CosDis;
save LNSUBRW_dis_gsSim LNSUBRW_dis_gsSim;
save LNSUBRW_lnc_gsSim LNSUBRW_lnc_gsSim;
save LNSUBRWCoslnc LNSUBRWCoslnc;
save LNSUBRWCosDis LNSUBRWCosDis;
...
for i=1:9
    ...
    h=strcat(h1,h2);
    simname = strcat('lnsubrw_lncsim',h);
...
for i=1:9
    ...
    h1=num2str(h(1));h2=num2str(h(2));
    h=strcat(h1,h2);
    simname = strcat('lnsubrw_dissim',h);
    eval([simname '=sim']);
    save([simname],[simname])  %±£´æ±ä»¯µÄ±äÁ¿Ãû
...
edge_f1 = [edge_f12;edge_f11];
xlswrite(['LNSUBRWcasestudy.xlsx'],edge_f1,'Sheet 1');

% case result
predY =xlsread('predYlnsubrw.xlsx');
[~,LNSUBRWlncrna] = xlsread('LDA-LNSUBRW-lncRNA_115.xlsx','Sheet1');
[~,LNSUBRWdisease] = xlsread('LDA-LNSUBRW-disease_178.xlsx','Sheet1');
