# -*- coding: utf-8 -*-
"""
Created on Fri Feb 19 14:16:51 2021
    ...
    for ii in range(0,7,1):
        for jj in range(0,2,1):
            for kk in range(0,3,1):
                ...
                EPOCHS = Epochs[ii]
                DROPOUT_RATIO = Dropout[jj]  
                LEARNING_RATE = Learning[kk]   
                ...
                namepkl = 'LDNFSGB_case10cv'+lncweit[iii]+disweit[iii]+'epoch'+str(EPOCHS)+'drop'+str(DROPOUT_RATIO)+'lr'+str(LEARNING_RATE)+'.pkl'
                PKL.append(namepkl)
...
#保存数据
import pickle
output = open('LDNFSGB_case10cv_para.pkl','wb')
...
import pickle
pkl_file= open('LDNFSGB_case10cv_para.pkl','rb')
