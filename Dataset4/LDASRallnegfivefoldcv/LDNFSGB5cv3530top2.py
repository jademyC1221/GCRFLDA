...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 24 22:56:47 2020
...
#                            side_hidden_dim=10, encode_hidden_dim=75, num_basis=2)
        ...
        self.encoder = StackGCNEncoder2(input_dim, gcn_hidden_dim, num_support,num_user,num_movie,)
        self.dense1 = FullyConnected(side_feat_dim, side_hidden_dim, dropout=0.,
                                     use_bias=True)
        self.dense2 = FullyConnected(gcn_hidden_dim + side_hidden_dim, encode_hidden_dim,
                                     dropout=DROPOUT_RATIO, activation=lambda x: x)
...
##    j=1
#    filename='/home/lsz/GraphNeuralNetwork-master/LDNFSGB/LDNFSGBedgecv5_3530/edge_f_ldnfsgb_cv3530_'+str(j)+'.xlsx'
...
##保存数据
#import pickle
#output = open('LDNFSGBfivefolddata3530.pkl','wb')
...
#读取数据
import pickle
pkl_file=open('LDNFSGBfivefolddata3530.pkl','rb')
...
#                    print('test on..............lnciii=%d,disjjj=%d,i=%d' % (iii,jjj,i))
    #                    print('test on....................%d' % i)
            ...
            name = 'LDNFSGB5cv_19'+'epoch'+str(EPOCHS)+'drop'+str(DROPOUT_RATIO)+'lr'+str(LEARNING_RATE)+'.pkl'
            PKL.append(name)
            ...
            #保存数据
    #                import pickle
            output = open('LDNFSGB5cv_19'+'epoch'+str(EPOCHS)+'drop'+str(DROPOUT_RATIO)+'lr'+str(LEARNING_RATE)+'.pkl','wb')
            ...
            output = open('LDNFSGB5cv_para.pkl','wb')
...
#读取数据
import pickle
pkl_file=open('LDNFSGB5cv_para.pkl','rb')
...
#print(lab)
#std_aupr = np.std(auprcs)
#plt.plot(mean_recall, mean_precision, label=r'Mean PR (AUPR = %0.2f $\pm$ %0.2f)' % (mean_aupr, std_aupr),
#         lw=2, alpha=.8)
#plt.show()
#plt.savefig('psr405_1.png')
