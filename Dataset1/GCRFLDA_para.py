...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan  4 00:50:03 2021
...
NUM_BASIS = 2
WEIGHT_DACAY = 0.
NODE_INPUT_DIM = lnc_identity_feature.shape[1]  #节点总数量
        ...
        for jj in range(0,2,1):
            for kk in range(0,3,1):
                ...
                #            prs.append(interp(mean_fpr, fpr, tpr))
                            ...
                            print('test on....................iii=%d,jjj=%d,tt=%d,i=%d' % (iii,jjj,tt,i))
                #                    print('test on....................%d' % i)
                    ...
                    namep='p2697crf1_meanpr'+lncweit[iii]+disweit[iii]+'_'+str(jjj)+'epoch'+str(EPOCHS)+'drop'+str(DROPOUT_RATIO)+'lr'+str(LEARNING_RATE)+'.pkl'
                    PKL.append(namep)
...
#                    import pickle
 ...
 #    import pickle 保存数据
output = open('main2697sim2crx1top4.pkl','wb')
pickle.dump(Weight,output)
pickle.dump(Weight1,output)
pickle.dump(PKL,output)
pickle.dump(AUCresult1,output)
pickle.dump(AUPRresult1,output)
output.close()

# #  读取数据
#import pickle
#pkl_file = open('main2697sim2crx1top4.pkl','rb')
